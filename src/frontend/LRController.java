...
public class LRController implements Initializable {
    ...
    void loginOnClick(ActionEvent event) throws SQLException, IOException {
        ...
        if(user.next() && email != ""){
            ...
            if(!user.getBoolean("isAdmin")) root = FXMLLoader.load(getClass().getResource("user/UserDashboard.fxml")); //load file if user
            else root = FXMLLoader.load(getClass().getResource("admin/AdminDashboard.fxml")); // load file if admin
    ...
    void registerOnClick(ActionEvent event) throws SQLException, IOException, Exception{
        ...
        if(pass.equals(pass2) && !db.query("SELECT * FROM USER where email='"+email+"'").next()){
